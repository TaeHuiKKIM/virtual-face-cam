...
이 저장소를 사용하세요. OBS Studio의 가상 카메라 기능이 필요합니다.

### OBS 없이 앱 자체만으로 가상 카메라를 만들고 싶다

아래 macOS 전용 네이티브 프로젝트를 보세요. Swift + CoreMediaIO Camera Extension으로
OBS 없이 `Virtual Face Cam` 카메라 장치를 만드는 버전입니다.

https://github.com/TaeHuiKKIM/virtual-face-cam-mac

단, macOS System Extension이라 Apple Developer Team, App Group, 최초 승인 과정이 필요합니다.
...
### OBS 없이 독립 앱으로 쓰고 싶어요

OBS 없이 앱 자체 Camera Extension으로 가상 카메라를 만들려면 아래 macOS 네이티브
프로젝트를 사용하세요.
